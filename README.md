...
2. Abrir `http://localhost:8091` y completar el perfilado: nombre, modo, clases, dias, objetivo y presupuesto. El paso 7 permite escribir requisitos personales opcionales (hasta 2000 caracteres), enviados al planner en `constraints.additionalRequirements`. Pulsar **Ver recomendaciones** desde ese paso: aparece la carga antes del calculo local y la llamada a Foundry. No hay acceso rapido de demostracion. En el paso 8, comparar rutas sobre un mapa real con destinos numerados y trazados amarillos, sin leyenda de progreso. Una instalacion con Camino activo lo conserva; para ensayar un perfil nuevo sin borrarlo, usar una ventana privada.
...
El planificador local ahora redistribuye recorridos a pie y en bici entre localidades cartografiadas a menos de 350 m del trazado original. Equilibra distancia y esfuerzo entre los dias solicitados, pudiendo dividir, combinar o recortar etapas del catalogo sin inventar geometria. Los objetivos orientativos son 40/55/70 km en bici y 18/23/28 km a pie para perfiles tranquilo/moderado/deportivo; conserva la dificultad original y rechaza repartos con jornadas mayores que 1,5 veces ese objetivo. Si existen repartos validos con el numero exacto de dias, no ofrece como equivalentes las plantillas de duracion distinta. El modo coche conserva su comportamiento anterior. Las jornadas nuevas se reconstruyen desde identificadores versionados `planned-v1`, por lo que mapa, simulacion y persistencia siguen usando el mismo recorrido tras una recarga. Los Caminos ya guardados no cambian.

Cada sugerencia muestra el `rationale` completo del agente en un parrafo de hasta 120 palabras, sin explicacion local antepuesta. `stageAdaptation` incluye ahora `stageBoundariesChanged`, `originalStages`, `dailyStages`, `changes`, `paceTargetKm` y `paceReason` para explicar el antes/despues real. La app calcula el reparto; Foundry ordena candidatos y redacta la justificacion. Los requisitos libres se envian al agente para evaluar el encaje, pero no son restricciones numericas interpretadas por el algoritmo local. Actualizar manualmente el bloque del prompt segun [los cambios para ultreia-planner](foundry/agents/ultreia-planner-changes.md); no se ha modificado el agente publicado. La prueba real de Foundry ya reconocio las divisiones y recortes con el nuevo contexto. La compatibilidad con respuestas antiguas sin `rationale` se mantiene mediante `reasons` y `tradeoffs`.

La redistribucion no comprueba plazas de alojamiento, accesibilidad actual ni condiciones de seguridad. Conserva las variantes del trazado de cada modo y solo usa geometria que pasa la auditoria local. Pruebas: seis etapas ciclistas del Primitivo repartidas en siete jornadas, progreso sobre todos sus segmentos y cierre con siete diarios; navegador aislado con seleccion, recarga y simulacion de las dos primeras jornadas, sin errores de pagina. No sustituye una validacion de campo o en dispositivo fisico.
