...
Cada sugerencia del planner muestra un unico parrafo de hasta 120 palabras con el tramo, las adaptaciones, los motivos segun el perfil y el compromiso principal. La peticion incluye `explanationGuidance` y datos `routeSelection` para distinguir un cambio de reparto de etapas de un recorte de la ruta base. Se admite `rationale` o, por compatibilidad con el agente publicado, se combinan sus motivos y compromisos existentes. No es necesario modificar el prompt publicado para mostrar el formato ampliado; las respuestas reales del ensayo usaron `reasons` y `tradeoffs`.
